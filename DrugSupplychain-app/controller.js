...
return{
	get_all_drug: function(req, res){
		...
		var peer = fabric_client.newPeer('http://localhost:7051');
	...
	add_drug: function(req, res){
		console.log("submit recording of a drug creation: ");
		...
		var peer = fabric_client.newPeer('http://localhost:7051');
		channel.addPeer(peer);
		var order = fabric_client.newOrderer('http://localhost:7050')
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    const request = {
		        chaincodeId: 'drug-app',
		        fcn: 'recordDrug',
		        args: [key, date, location, timestamp, holder],
		        chainId: 'mychannel',
		        txId: tx_id
		    };
		    ...
		    if (isProposalGood) {
		        ...
		        event_hub.setPeerAddr('http://localhost:7053');
	...
	change_holder: function(req, res){
		console.log("changing holder of drug drug: ");
